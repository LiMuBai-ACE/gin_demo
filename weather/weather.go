...
type Weather struct {
	...
	nbytes                            int64
	weatherlru                        *middleware.Cache
	entryCache                        *middleware.Cache
	nhit, nget                        int64
	nevict                            int64
...
func New(maxEntries int) *Weather {
	return &Weather{
		weatherlru: middleware.New(maxEntries),
		entryCache: middleware.New(0),
...
func (c *Weather) addWeatherInfoToCache(key string, value *WeatherInfo) {
	...
	if c.weatherlru.OnEvicted == nil {
		c.weatherlru.OnEvicted = func(key middleware.Key, value interface{}) {
